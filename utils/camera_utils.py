...
def get_orbit_poses_around_center(
    poses: Float[Tensor, "num_poses 3 4"],
    Ks: Float[Tensor, "num_poses 3 3"],
    num_frames: int = 16,
) -> Tuple[Float[Tensor, "num_frames 3 4"], Float[Tensor, "num_frames 3 3"]]:
    """Build a single-loop orbit trajectory that stays in-distribution w.r.t. inputs.

    The orbit circles around the mean of the input camera centers, rotating about
    the world +Y axis. To keep the rendered views close to the training
    distribution we derive the orbit geometry from the input cameras:
      - orbit center  = mean of input camera centers
      - orbit radius   = median XZ-plane distance of inputs to the orbit center
      - orbit height   = median input camera Y
      - intrinsics     = mean of input intrinsics (shared across frames)
    All cameras look at the orbit center, using the OpenCV convention
    (camera x=right, y=down, z=forward).

    Args:
        poses: input camera-to-world poses, OpenCV convention [num_poses, 3, 4].
        Ks: input intrinsics [num_poses, 3, 3].
        num_frames: number of poses to generate around the loop.

    Returns:
        tuple of orbit poses [num_frames, 3, 4] and intrinsics [num_frames, 3, 3].
    """
    device = poses.device
    poses = poses.to(torch.float32)
    Ks = Ks.to(torch.float32)

    centers = poses[:, :3, 3]  # [v, 3]
    orbit_center = centers.mean(dim=0)  # [3]

    offsets = centers - orbit_center[None]  # [v, 3]
    horiz_dist = torch.sqrt(offsets[:, 0] ** 2 + offsets[:, 2] ** 2)  # XZ-plane radius
    radius = torch.median(horiz_dist)
    # Guard against a degenerate (near-zero) radius when inputs are co-located.
    if float(radius) < 1e-6:
        radius = torch.median(torch.linalg.norm(offsets, dim=-1)).clamp_min(1e-3)
    height = torch.median(centers[:, 1])

    K_mean = Ks.mean(dim=0)  # [3, 3]

    # Down vector so the generated camera y-axis points down (OpenCV convention).
    up_world = torch.tensor([0.0, -1.0, 0.0], device=device, dtype=torch.float32)

    # Start the loop aligned with the first input camera's azimuth for continuity.
    theta0 = torch.atan2(offsets[0, 2], offsets[0, 0])

    traj, k_interp = [], []
    for i in range(num_frames):
        theta = theta0 + 2.0 * math.pi * i / num_frames
        eye = torch.stack([
            orbit_center[0] + radius * torch.cos(theta),
            height,
            orbit_center[2] + radius * torch.sin(theta),
        ])

        z_axis = normalize(orbit_center - eye)  # forward
        x_axis = normalize(torch.cross(up_world, z_axis, dim=0))  # right
        y_axis = torch.cross(z_axis, x_axis, dim=0)  # down

        c2w = torch.stack([x_axis, y_axis, z_axis, eye], dim=1)  # [3, 4]
        traj.append(c2w)
        k_interp.append(K_mean)
