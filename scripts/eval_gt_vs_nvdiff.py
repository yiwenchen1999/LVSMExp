#!/usr/bin/env python3
"""
Compute PSNR, SSIM, LPIPS between GT (with alpha) and NVDiff predictions.
- Both composited on black background (GT alpha applied to pred as well).
- Match: pred view_00030.png <-> GT 00030.png
...
def find_pred_for_gt(gt_fn, pred_dir):
    """Return pred filename for gt_fn (e.g. 00030.png). Tries view_00030.png then 00030.png."""
    base = gt_fn[:-4] if gt_fn.endswith(".png") else gt_fn
    for candidate in [f"{PRED_PREFIX}{base}.png", f"{base}.png"]:
        if os.path.isfile(os.path.join(pred_dir, candidate)):
            return candidate
    return None
...
def eval_one_scene(gt_dir, pred_dir, tone_map=False, masked=False, lpips_fn=None):
    ...
    for gt_fn in gt_files:
        pred_fn = find_pred_for_gt(gt_fn, pred_dir)
        if pred_fn is None:
            continue
        ...
        h_gt, w_gt = gt_black.shape[:2]
        if pred_black.shape[0] != h_gt or pred_black.shape[1] != w_gt:
            pred_black = np.array(
                Image.fromarray((pred_black * 255).astype(np.uint8)).resize((w_gt, h_gt), Image.LANCZOS)
...
def main():
    ...
    parser.add_argument("nvdiff_dir", help="NVDiff directory")
    parser.add_argument("--all", action="store_true", help="Run for all scene pairs")
    parser.add_argument("--tone-map", action="store_true", help="Apply optimal linear scale to pred on foreground")
    parser.add_argument("--masked", action="store_true", help="Metrics only over foreground (default: whole image, black bg)")
    ...
    nvdiff_root = os.path.abspath(args.nvdiff_dir)
    if not os.path.isdir(gt_root):
        print(f"GT dir not found: {gt_root}")
        sys.exit(1)
    if not os.path.isdir(nvdiff_root):
        print(f"NVDiff dir not found: {nvdiff_root}")
        sys.exit(1)
    ...
    if args.all:
        ...
        pred_subdirs = [d for d in os.listdir(nvdiff_root) if os.path.isdir(os.path.join(nvdiff_root, d))]
        scene_names = sorted(set(gt_subdirs) & set(pred_subdirs))
        if not scene_names:
            print("No common scene subdirs.")
            sys.exit(1)
        print(f"Mode: {'masked' if args.masked else 'whole image'} (black bg). Tone mapping: {'on' if args.tone_map else 'off'}")
        print(f"Match: view_XXXXX.png <-> XXXXX.png")
        ...
        for name in scene_names:
            ...
            pred_dir = os.path.join(nvdiff_root, name)
            out = eval_one_scene(gt_dir, pred_dir, tone_map=args.tone_map, masked=args.masked, lpips_fn=lpips_fn)
            if out is None:
                print(f"  [skip] {name}: no matching view_*.png files")
                continue
            all_results[name] = {"n": out["n"], "avg_psnr": out["avg_psnr"], "avg_ssim": out["avg_ssim"], "avg_lpips": out["avg_lpips"]}
        ...
        print("Overall:")
        print(f"  PSNR:  {overall_psnr:.4f}" if overall_psnr else "  PSNR:  N/A")
        print(f"  SSIM:  {overall_ssim:.4f}" if overall_ssim else "  SSIM:  N/A")
        print(f"  LPIPS: {overall_lpips:.6f}" if overall_lpips else "  LPIPS: N/A")

        if args.output:
            import json
            with open(args.output, "w") as f:
                json.dump({"scenes": all_results, "overall": {"psnr": overall_psnr, "ssim": overall_ssim, "lpips": overall_lpips, "n_scenes": n_scenes}}, f, indent=2)
            print(f"\nSaved to {args.output}")
        return

    out = eval_one_scene(gt_root, nvdiff_root, tone_map=args.tone_map, masked=args.masked, lpips_fn=lpips_fn)
    if out is None:
        print("No matching view_*.png files found.")
        sys.exit(1)
    results = out["per_image"]
    print(f"GT dir: {gt_root}")
    print(f"NVDiff dir: {nvdiff_root}")
    print(f"Mode: {'masked' if args.masked else 'whole image'} (black bg). Tone mapping: {'on' if args.tone_map else 'off'}")
