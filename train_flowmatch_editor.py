...
single_step_editor_ckpt = config.training.get("single_step_editor_checkpoint", "")
if single_step_editor_ckpt:
    print(f"------2.1 Initializing from Single Step Editor at {single_step_editor_ckpt}------")
    result = model.init_from_singleStepEditor(single_step_editor_ckpt)
    if result is None:
        print(f"Warning: Failed to initialize from Single Step Editor, falling back to LVSM initialization if available")
    else:
        print(f"------2.2 Successfully initialized from Single Step Editor------")

if config.training.get("LVSM_checkpoint_dir", "") and not single_step_editor_ckpt:
