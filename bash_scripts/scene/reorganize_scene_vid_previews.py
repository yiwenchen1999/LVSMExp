#!/usr/bin/env python3
"""
Reorganize scene_vid previews where supervision layout is gt|pred (no relit_gt).

Input layout:
  result_previews/scene_vid/iter_XXXXXXXX/
    input_<scene>.jpg
...
Output:
  result_previews/scene_vid_flattened/<scene>_iter_XXXXXXXX.jpg
    3-row montage: input strip (scaled), gt strip, pred strip.

  result_previews/scene_vid_single_image/<scene>/iter_XXXXXXXX/
    input_view_00..N-1.jpg
    camera_poses.json + camera_context_view_XX.json (if source exists)
    frame_0000/view_gt.jpg, view_pred.jpg, camera_target.json (if source exists)
    frame_0001/...
"""
...
_DEFAULT_BASE = _REPO_ROOT / "result_previews/scene_vid"
...
def split_supervision_chunk(path: Path) -> list[tuple[Image.Image, Image.Image]]:
    """
    Split chunk where each frame contributes 2 square tiles: gt|pred.
    """
    im = open_rgb(path)
    w, h = im.size
    tile = h
    ...
    n_tiles = w // tile
    if n_tiles % 2 != 0:
        raise SkipScene(f"{path.name}: tile count {n_tiles} is not even (expected gt|pred pairs)")
    out: list[tuple[Image.Image, Image.Image]] = []
    for i in range(n_tiles // 2):
        x0 = i * 2 * tile
        gt = im.crop((x0, 0, x0 + tile, h))
        pred = im.crop((x0 + tile, 0, x0 + 2 * tile, h))
        out.append((gt, pred))
    return out


def load_frames(parts: list[tuple[int, int, Path]]) -> list[tuple[Image.Image, Image.Image]]:
...
def process_scene(
    ...
    if do_flattened:
        ...
        row_gt = hstack([fr[0] for fr in frames])
        row_pred = hstack([fr[1] for fr in frames])
    ...
    if do_single:
        ...
        for i, (gt, pred) in enumerate(frames):
