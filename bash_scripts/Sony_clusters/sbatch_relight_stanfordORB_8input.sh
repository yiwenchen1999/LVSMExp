#!/bin/bash
#SBATCH --job-name=relight_sORB_8in
#SBATCH --partition=ct
#SBATCH --account=ct
#SBATCH --nodes=1
#SBATCH --gres=gpu:2
...
export CKPT_DIR="/music-shared-disk/group/ct/yiwen/codes/LVSMExp/ckpt/relight_stanfordORB_8input"
...
############################
# Run training – 8 input views, 8 target views, 16 total
############################
srun singularity exec --nv $BIND $SIF bash -lc "
  ...
  torchrun --nproc_per_node 2 --nnodes 1 \
    ...
    training.wandb_exp_name = LVSM_edit_stanfordORB_8input \
