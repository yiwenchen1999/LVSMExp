#!/bin/bash
#SBATCH --job-name=relight_sORB_16in
...
export DATA_LIST="/music-shared-disk/group/ct/yiwen/data/stanford_ORB_lvsm/test/full_list_gnome.txt"
export CKPT_DIR="/music-shared-disk/group/ct/yiwen/codes/LVSMExp/ckpt/relight_stanfordORB_16input_gnomes"
...
############################
# Run training – 16 input views, 8 target views, 24 total
############################
srun singularity exec --nv $BIND $SIF bash -lc "
  ...
  torchrun --nproc_per_node 1 --nnodes 1 \
    ...
    training.batch_size_per_gpu = 3 \
    training.checkpoint_dir = \"$CKPT_DIR\" \
    training.LVSM_checkpoint_dir = \"$LVSM_CKPT_DIR\" \
    training.wandb_exp_name = LVSM_edit_stanfordORB_16input \
