...
#SBATCH --job-name=relight_flowmatch_noise2relit_lr1e6
#SBATCH --mem=128
#SBATCH --ntasks=32
#SBATCH --output=myjob.relight_flowmatch_noise2relit_lr1e6.out
#SBATCH --error=myjob.relight_flowmatch_noise2relit_lr1e6.err
...
torchrun --nproc_per_node 1 --nnodes 1 \
    --rdzv_id 18637 --rdzv_backend c10d --rdzv_endpoint localhost:29502 \
    train_flowmatch_editor.py --config configs/LVSM_flow_match_editor_noise2relit.yaml \
    training.batch_size_per_gpu = 4 \
    training.checkpoint_dir = ckpt/LVSM_flow_match_editor_noise2relit_lr1e6 \
    training.LVSM_checkpoint_dir = ckpt/LVSM_scene_encoder_decoder \
    training.wandb_exp_name = LVSM_flowmatch_noise2relit_dense_lr1e6_singleMap \
