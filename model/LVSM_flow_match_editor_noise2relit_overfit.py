...
class FlowMatchEditor(LatentSceneEditor):
    ...
    def forward(self, data_batch, timestep=None, skip_renderer=True):
        ...
        # Options: 'flow_only', 'render_only', 'both'
        training_mode = self.config.training.get("training_mode", "both")
        use_flow_loss = training_mode in ["flow_only", "both"]
        use_render_loss = training_mode in ["render_only", "both"]
        
        if not skip_renderer and use_render_loss:
            ...
            if hasattr(target, 'relit_images'):
                ...
                # Combine flow loss and reconstruction loss based on training mode
                # Get weights from config (default to 1.0 if not specified)
                flow_loss_weight = self.config.training.get("flow_loss_weight", 1.0) if use_flow_loss else 0.0
                reconstruction_loss_weight = 1.0 if use_render_loss else 0.0  # Reconstruction loss already has internal weights
                ...
                loss_metrics.reconstruction_loss = reconstruction_loss_metrics.loss if use_render_loss else torch.tensor(0.0, device=loss_flow.device)
                loss_metrics.l2_loss = reconstruction_loss_metrics.l2_loss if use_render_loss else torch.tensor(0.0, device=loss_flow.device)
                loss_metrics.lpips_loss = reconstruction_loss_metrics.lpips_loss if use_render_loss else torch.tensor(0.0, device=loss_flow.device)
                loss_metrics.perceptual_loss = reconstruction_loss_metrics.perceptual_loss if use_render_loss else torch.tensor(0.0, device=loss_flow.device)
                loss_metrics.psnr = reconstruction_loss_metrics.psnr if use_render_loss else torch.tensor(0.0, device=loss_flow.device)
            else:
                # If no relit_images, only use flow loss (if enabled)
                loss_metrics.loss = loss_flow if use_flow_loss else torch.tensor(0.0, device=loss_flow.device)
