...
class Images2LatentScene(nn.Module):
    ...
    def forward(self, data_batch, has_target_image=True):
        ...
        #& Step 17: Process albedo decoder if enabled
        rendered_albedos = None
        if self.use_albedo_decoder:
            # Apply albedo decoder to the same decoder input tokens
            albedo_decoder_input_tokens = decoder_input_tokens  # Reuse the same input
            transformer_albedo_output_tokens = self.pass_layers(
                self.transformer_decoder_albedo, 
                albedo_decoder_input_tokens, 
                gradient_checkpoint=True, 
                checkpoint_every=checkpoint_every
            )
            
            # Extract target albedo tokens
            target_albedo_tokens, _ = transformer_albedo_output_tokens.split(
                [n_patches, n_latent_vectors], dim=1
            ) # [b*v_target, n_patches, d], [b*v_target, n_latent_vectors, d]
            
            # Decode albedo tokens using the same image token decoder
        ...
        #& Step 18: Compute loss (if target images are provided)
