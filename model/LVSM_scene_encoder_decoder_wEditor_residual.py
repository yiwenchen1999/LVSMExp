...
class LatentSceneEditor(nn.Module):
    ...
    def forward(self, data_batch, has_target_image=True):
        ...
        rendered_images = self.renderer(latent_tokens_final, target, n_patches, d)
        
        #& Step 4.5: Process albedo decoder if enabled
        rendered_albedos = None
        if self.use_albedo_decoder:
            rendered_albedos = self.renderer_albedo(latent_tokens_final, target, n_patches, d)
        ...
        if has_target_image:
            ...
            if self.use_albedo_decoder and hasattr(target, 'albedos') and target.albedos is not None:
                ...
                # Combine losses: add image loss and albedo loss and fidelity loss
                combined_loss = image_loss_metrics.loss + albedo_loss_metrics.loss + fidelity_loss
                ...
                combined_loss = image_loss_metrics.loss + fidelity_loss
                loss_metrics = image_loss_metrics
                loss_metrics.loss = combined_loss
                loss_metrics.fidelity_loss = fidelity_loss
        else:
            loss_metrics = None
