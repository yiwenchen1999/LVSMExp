...
class XPredictionEditor(LatentSceneEditor):
    ...
    def forward(self, data_batch, timestep=None, skip_renderer=True):
        ...
        # Extract predicted x (z_B)
        n_latent_vectors = self.config.model.transformer.n_latent_vectors
        pred_x = editor_output_tokens[:, :n_latent_vectors, :]
        ...
        if not skip_renderer:
            ...
            if hasattr(target, 'relit_images'):
                ...
                loss_metrics.reconstruction_loss = torch.tensor(0.0, device=loss_mse.device)
                loss_metrics.l2_loss = torch.tensor(0.0, device=loss_mse.device)
                loss_metrics.lpips_loss = torch.tensor(0.0, device=loss_mse.device)
                loss_metrics.perceptual_loss = torch.tensor(0.0, device=loss_mse.device)
                loss_metrics.psnr = torch.tensor(0.0, device=loss_mse.device)
        else:
